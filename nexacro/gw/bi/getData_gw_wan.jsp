...
<%
	...
	String strstore		  =  inVl.getString("store");
	String strPageNo  	  =  inVl.getString("PageNo");
	String strPageSize	  =  inVl.getString("PageSize");
	String strStatusGb	  =  inVl.getString("strStatusGb");
	String strStatusGb01  =  inVl.getString("strStatusGb01");
	
	try {
	  
	  Callstmt = conn.prepareCall("{Call GROUPWARE.PKG_I_FLOW.PROC_FLOW_WAN(?,?,?,?,?,?,?,?)}");
	  Callstmt.registerOutParameter(1, TbTypes.CURSOR);
	  Callstmt.setString(2,strgubun);
	  Callstmt.setString(3,strsabun);
	  Callstmt.setString(4,strstore);
	  Callstmt.setString(5,strPageNo);
      Callstmt.setString(6,strPageSize);
	  Callstmt.setString(7,strStatusGb);
	  Callstmt.setString(8,strStatusGb01);
	  Callstmt.execute();
